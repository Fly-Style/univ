import sun.jvm.hotspot.tools.SysPropsDumper;
import sun.security.util.BitArray;

import java.io.InputStream;
import java.io.OutputStream;

/**
 * Created by MakeYouHappy on 24.09.2015.
 */

public class Rijndael
{
    private String key;
    private String input;

    private byte[] byteKey;
    private byte[] inputBytes;
    private int[] intKey;
    private int[][] deployedKey;

    private int[][] state;
    private int[][] output;
    int [] sbox = {
                
    };

    public static final int[][] invsbox = {
            {0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb},
            {0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb},
            {0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42, 0xfa, 0xc3, 0x4e},
            {0x08, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25},
            {0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, 0xa4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92},
            {0x6c, 0x70, 0x48, 0x50, 0xfd, 0xed, 0xb9, 0xda, 0x5e, 0x15, 0x46, 0x57, 0xa7, 0x8d, 0x9d, 0x84},
            {0x90, 0xd8, 0xab, 0x00, 0x8c, 0xbc, 0xd3, 0x0a, 0xf7, 0xe4, 0x58, 0x05, 0xb8, 0xb3, 0x45, 0x06},
            {0xd0, 0x2c, 0x1e, 0x8f, 0xca, 0x3f, 0x0f, 0x02, 0xc1, 0xaf, 0xbd, 0x03, 0x01, 0x13, 0x8a, 0x6b},
            {0x3a, 0x91, 0x11, 0x41, 0x4f, 0x67, 0xdc, 0xea, 0x97, 0xf2, 0xcf, 0xce, 0xf0, 0xb4, 0xe6, 0x73},
            {0x96, 0xac, 0x74, 0x22, 0xe7, 0xad, 0x35, 0x85, 0xe2, 0xf9, 0x37, 0xe8, 0x1c, 0x75, 0xdf, 0x6e},
            {0x47, 0xf1, 0x1a, 0x71, 0x1d, 0x29, 0xc5, 0x89, 0x6f, 0xb7, 0x62, 0x0e, 0xaa, 0x18, 0xbe, 0x1b},
            {0xfc, 0x56, 0x3e, 0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4},
            {0x1f, 0xdd, 0xa8, 0x33, 0x88, 0x07, 0xc7, 0x31, 0xb1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xec, 0x5f},
            {0x60, 0x51, 0x7f, 0xa9, 0x19, 0xb5, 0x4a, 0x0d, 0x2d, 0xe5, 0x7a, 0x9f, 0x93, 0xc9, 0x9c, 0xef},
            {0xa0, 0xe0, 0x3b, 0x4d, 0xae, 0x2a, 0xf5, 0xb0, 0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61},
            {0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d}
    };


    int [] revbox =    {
                
    };

    public static final int[][] galois = {
            {0x02, 0x03, 0x01, 0x01},
            {0x01, 0x02, 0x03, 0x01},
            {0x01, 0x01, 0x02, 0x03},
            {0x03, 0x01, 0x01, 0x02}
    };

    public static final int[][] invgalois = {{0x0e, 0x0b, 0x0d, 0x09},
            {0x09, 0x0e, 0x0b, 0x0d},
            {0x0d, 0x09, 0x0e, 0x0b},
            {0x0b, 0x0d, 0x09, 0x0e}};

    // rCon is Round Constant used for the Key Expansion [1st col is 2^(r-1) in GF(2^8)] [§5.2]
    public static final int[][] rCon = {
        {0x00, 0x00, 0x00, 0x00},
        {0x01, 0x00, 0x00, 0x00},
        {0x02, 0x00, 0x00, 0x00},
        {0x04, 0x00, 0x00, 0x00},
        {0x08, 0x00, 0x00, 0x00},
        {0x10, 0x00, 0x00, 0x00},
        {0x20, 0x00, 0x00, 0x00},
        {0x40, 0x00, 0x00, 0x00},
        {0x80, 0x00, 0x00, 0x00},
        {0x1b, 0x00, 0x00, 0x00},
        {0x36, 0x00, 0x00, 0x00}
};

    private int arrLen = 4;

    Rijndael(String key, String input) {
        try {
            this.key = key;
            this.input = input;

            byte[] byteKey = key.getBytes();
            this.byteKey = byteKey;
            this.intKey = new int[arrLen * (arrLen + 6)];
            this.inputBytes = input.getBytes();
            state = new int[arrLen][arrLen];

            System.out.println(inputBytes.length);
            System.out.println("----------------");

            for (int i = 0; i < byteKey.length; i++) {
                intKey[i] = (int) byteKey[i];
            }

            this.deployedKey = KeyExpansion(intKey);

            for (int i = 0; i < arrLen; ++i) {
                for (int j = 0; j < arrLen; ++j) {
                    state[i][j] = inputBytes[i * arrLen + j];
                }
            }
        } catch (Exception anyOther) {
            anyOther.printStackTrace();


        }
    }

    private int[][] subKey(int[][] km, int begin) {
        int[][] arr = new int[arrLen][arrLen];
        for (int i = 0; i < arr.length; i++) {
            for (int j = 0; j < arr.length; j++) {
                arr[i][j] = km[arrLen*begin + i][j];
            }
        }
        return arr;
    }

    private int[][] KeyExpansion(int[] key) {
    int roundsNumber = arrLen + 6;

    int[][] res = new int[arrLen*roundsNumber][arrLen];

    int[] temp = new int[arrLen];

    // initialise first Nk words of expanded key with cipher key
    for (int i = 0; i < roundsNumber; i++) {
        int[] keyBlock = {key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]};
        res[i] = keyBlock;
    }

    // expand the key into the remainder of the schedule
    for (int i = roundsNumber; i < arrLen*roundsNumber; i++) {

        for (int t = 0; t < 4; t++)
            temp[t] = res[i-1][t];
        // each Nk'th word has extra transformation
        if (i % roundsNumber == 0) {
            RotWord(temp);
            SubWord(temp);
            for (int t=0; t<4; t++)
                temp[t] ^= rCon[i/roundsNumber][t];
        }

        // xor w[i] with w[i-1] and w[i-Nk]
        for (int t = 0; t < 4; t++)
            res[i][t] = res[i-roundsNumber][t] ^ temp[t];
    }

    return res;
}

    private int[][] AddRoundKey(int k) {

        int[][]res = subKey(deployedKey, k);

        for(int i = 0; i < arrLen; i++)
            for (int j = 0; j < arrLen; j++) {
                res[j][i] ^= state[j][i];
                //res[i][j] = state[i][j] ^ this.deployedKey[k][j]; // HERE IS ERROR
            }
        return res;
    }
    public int[][] AddRoundKey(int[][] bytematrix, int[][] keymatrix) {
        for (int i = 0; i < bytematrix.length; i++) {
            for (int j = 0; j < bytematrix[0].length; j++) {
                bytematrix[j][i] ^= keymatrix[j][i];
            }
        }
        return bytematrix;
    }


    private int[][] SubBytes (int [][] mtrx) {
       int[][] res = new int [arrLen][arrLen];

        for (int r = 0; r < arrLen; r++) {
            for (int c = 0; c < arrLen; c++)
                res[r][c] = sbox[mtrx[r][c]];
        }
        return res;
    }

    public int[][] InvSubBytes(int[][] mtrx) {
        int[][] res = new int [arrLen][arrLen];
        for (int r = 0; r < mtrx.length; r++) {
            for (int c = 0; c < mtrx[0].length; c++) {
                int hex = res[c][r];
                res[c][r] = invsbox[hex / 16][hex % 16];
            }
        }
        return res;
    }

    private int[] BackShift (int[] arr, int times) {
        if (arr.length == 0 || arr.length == 1 || times % 4 == 0) {
            return arr;
        }
        while (times > 0) {
            int temp = arr[arr.length - 1];
            for (int i = arr.length - 1; i > 0; i--) {
                arr[i] = arr[i - 1];
            }
            arr[0] = temp;
            --times;
        }
        return arr;
    }

    public int[][] InvShiftRows(int[][] arr) {
        int [][] res = new int[arrLen][arrLen];
        for (int i = 1; i < arr.length; i++) {

            res[i] = BackShift(arr[i], i);
        }
        return res;
    }

    private int[][] ShiftRows(int[][] mtrx) {
        int[][] newmtrx = new int[arrLen][arrLen];
                                                                            // no changes in first row;
        for (int r = 1; r<4; r++) {
            int[] t = new int[arrLen];
            for (int c = 0; c < 4; c++) t[c] = mtrx[r][(c+r) % arrLen];     // shift into temp copy
            for (int c = 0; c < 4; c++) newmtrx[r][c] = t[c];
        }
        return newmtrx;
    }

    public void MixColumns(int[][] mtrx) {
        int[][] arr = new int[arrLen][arrLen];
        for(int i = 0; i < 4; i++)
        {
            System.arraycopy(mtrx[i], 0, arr[i], 0, 4);
        }
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                mtrx[i][j] = mcHelper(arr, galois, i, j);
            }
        }
    }

    private int mcHelper(int[][] arr, int[][] galuas, int i, int j) {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = galuas[i][k];
            int b = arr[k][j];
            mcsum ^= mcCalc(a, b);
        }
        return mcsum;
    }

    private int mcCalc(int a, int b) {
        if (a == 1) {
            return b;
        } else if (a == 2) {
            return MCTables.mc2[b / 16][b % 16];
        } else if (a == 3) {
            return MCTables.mc3[b / 16][b % 16];
        }
        return 0;
    }

    public void InvMixColumns(int[][] arr) {
        int[][] tarr = new int[4][4];
        for(int i = 0; i < 4; i++)
        {
            System.arraycopy(arr[i], 0, tarr[i], 0, 4);
        }
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                arr[i][j] = InvMcHelper(tarr, invgalois, i, j);
            }
        }
    }

    private int InvMcHelper(int[][] arr, int[][] igalois, int i, int j) {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = igalois[i][k];
            int b = arr[k][j];
            mcsum ^= InvMcCalc(a, b);
        }
        return mcsum;
    }

    private int InvMcCalc(int a, int b) {
        if (a == 9) {
            return MCTables.mc9[b / 16][b % 16];
        } else if (a == 0xb) {
            return MCTables.mc11[b / 16][b % 16];
        } else if (a == 0xd) {
            return MCTables.mc13[b / 16][b % 16];
        } else if (a == 0xe) {
            return MCTables.mc14[b / 16][b % 16];
        }
        return 0;
    }

    private void SubWord (int[] word) {
        for (int i=0; i < 4; i++)
            word[i] = sbox[word[i]];
    }

    private void RotWord (int[] word) {
        int tmp = word[0];
        for (int i = 0; i < 3; i++)
            word[i] = word[i + 1];

        word[3] = tmp;
    }

    public int[][] Encrypt () {

        int[][] temp = state;
        temp = AddRoundKey(0);
        for (int i = 1; i < 9; i++) {
            int[][] T = SubBytes(temp);
            T = ShiftRows(T);
            MixColumns(T) ;
            AddRoundKey(i);
            temp = T;
        }
        temp = SubBytes(temp);
        temp = ShiftRows(temp);
        AddRoundKey(9);

        output = new int[arrLen][arrLen];
        this.output = temp;
        return output;
    }

    public int[][] Decrypt () {

        int temp[][] = this.output;
        temp = AddRoundKey(9);

        for (int round = 8; round >= 1; round--) {
            int[][] T = InvShiftRows(temp);
            T = InvSubBytes(T);
            T = AddRoundKey(round);
            InvMixColumns(T);
            temp = T;
        }

        temp = InvShiftRows(temp);
        temp = InvSubBytes(temp);
        temp = AddRoundKey(0);

        return temp;
    }

    public static String MatrixToString(int[][] m) {
        String t = "";
        for (int i = 0; i < m.length; i++) {
            for (int j = 0; j < m[0].length; j++) {
                String h = Integer.toHexString(m[j][i]).toUpperCase();
                if (h.length() == 1) {
                    t += '0' + h;
                } else {
                    t += h;
                }
            }
        }
        return t;
    }

    public static void main(String[] args) {
        String someText = "I`ll be back, TR";
        String txt = "1231241678999877";
        String key = "1123213123123123";
        Rijndael algo = new Rijndael(key, txt);

        int[][] ars = algo.Encrypt();
        System.out.println(ars);
        //String
        int[][] res = algo.Decrypt();
        String out = MatrixToString(res);

        System.out.println(out);

    }

}
